...
class ArduinoLabel(Scene):
    def construct(self):        
        ...
        line1 = Line(rect1.get_right(),label1.get_left(),color=YELLOW).shift(UP * 0.12)
        label1.shift(UP * 0.12)
        ...
        rect5 = SurroundingRectangle(resetbutton,color=MAROON).set_fill(opacity=0.4)
        label5 = Text("Reset按钮",color=MAROON,weight=BOLD).scale(0.4).next_to(label1,DOWN,aligned_edge=LEFT)
        label5.move_to((label5.get_x(),rect5.get_y(),0))
        line5 = Line(rect5.get_right(),label5.get_left(),color=MAROON).shift(DOWN * 0.4)
        label5.shift(DOWN * 0.4)
        labels5 = Paragraph("按一下，内部烧录的程序就会从\n头开始执行",color=MAROON).scale(0.3).next_to(label5,DOWN,aligned_edge=LEFT,buff=0.05)
        self.add(rect5,label5,labels5,line5)
        
        rect6 = SurroundingRectangle(rightpins[4],color=TEAL).set_fill(opacity=0.4)
        label6 = Text("GND",color=TEAL,weight=BOLD).scale(0.4).next_to(rect6,LEFT * 14)
        labels6 = Paragraph("电路的参考电位点，相当于电池\n的负极，是所有电压测量的基准\n上下两排均有GND引脚，共3个",color=TEAL,alignment="right").scale(0.3).next_to(label6,DOWN,aligned_edge=RIGHT,buff=0.05)
        ...
        rect8 = SurroundingRectangle(VGroup(leftpins[5],leftpins[6]),color=GOLD).set_fill(opacity=0.4)
        label8 = Text("3.3V/5V输出",color=GOLD,weight=BOLD).scale(0.4).next_to(label6,DOWN,aligned_edge=RIGHT)
        label8.move_to((label8.get_x(),rect8.get_y(),0))
        line8 = Line(rect8.get_left(),label8.get_right(),color=GOLD)
        labels8 = Paragraph("由板载稳压器提供，可对外输出\n3.3V或5V电压供外部模块使用",color=GOLD,alignment="right").scale(0.3).next_to(label8,DOWN,aligned_edge=RIGHT,buff=0.05)
